...
// 		let sz = 2_u32.pow(sz);
// 		let img = gen_image(sz);
// 		group.bench_with_input(BenchmarkId::new("cringe", sz), &img, |b, i| {
// 			let (w, h) = i.dimensions();
// 			let mut out: Vec<u8> = vec![0; (w*h*3) as usize];		
// 			let bytes = i.as_bytes();
// 			b.iter(|| cringe_dim(&bytes[..], &mut out[..]))
// 		});
// 		group.bench_with_input(BenchmarkId::new("simd4", sz), &img, |b, i| {
// 			let (w, h) = i.dimensions();
// 			let mut out: Vec<u8> = vec![0; (w*h*3) as usize];		
// 			let bytes = i.as_bytes();
// 			b.iter(|| simd4_dim(&bytes[..], &mut out[..]))
// 		});
// 	}
// }

criterion_group!(benches, bench_greys);
